...
void ProductManagerForm::loadData()
{
    ...
    while (!in.atEnd()) {
        ...
        if(row.size()) {
            int id = row[0].toInt();
            ProductItem* c = new ProductItem(id, row[1], row[2].toInt(), row[3].toInt());
...
void ProductManagerForm::on_addPushButton_clicked()
{
    ...
    if(name.length()) {
        ProductItem* c = new ProductItem(id, name, price, quantity);
        ui->treeWidget->addTopLevelItem(c);
        productList.insert(id, c);
...
//    for(int i = 0; i < ui->treeWidget->columnCount(); i++)
                   ...
                   : Qt::MatchCaseSensitive;
    {
        ...
        foreach(auto i, items) {
            ...
            ProductItem* item = new ProductItem(id, name, price, quantity);
